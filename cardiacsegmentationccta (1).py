...
n_iterations = 5000 #@param {type:"slider", min:100, max:30000, step:100}
...
get_ipython().system_raw('/usr/local/bin/python -m visdom.server -port 8099 >> visdomlog.txt 2>&1 &')
get_ipython().system_raw('lt --port 8099 >> url.txt 2>&1 &')
import time
time.sleep(5)
! cat url.txt
import visdom
...
try:
  # TODO: Replace shutil with os alternatives
...
try:
  if mode == 'test' and download_trained_networks:
...
def load_mhd_to_npy(filename):
    ...
    print("Image all zeros state: ", (np.swapaxes(sitk.GetArrayFromImage(image), 0, 2) == 0).all())
    ...
    for it in range(num_epochs):
        ...
        if it % itInt == 0 and it > 0:
            ...
            viz.image(np.clip(scndy.zoom(
                (np.rot90(np.flipud(np.squeeze(valims[:, :, 13])), k=3, axes=(0, 1)) + 0.25) * 255,
                1.0), 0.0, 255.0),
                win=1,
            ...
            viz.image(np.clip(
                scndy.zoom((np.rot90(np.flipud(np.squeeze(valla[:, :, 13])), k=3, axes=(0, 1)) + 0.25) * 35, 1.0),
                0.0, 255.0),
                win=2,
...
def test3D(netnames, imdir):
    ...
    if len(filenames) > 0:
        for filename in filenames:
            ...
            for k in range(NCLASS):
                ...
                outim_os_or[k, :, :, :] = scndy.zoom(outim_os[k, :, :, :], (
                    orshape[0] / newshape[0], orshape[1] / newshape[1], orshape[2] / newshape[2]),
                                ...
                                # TODO: DEBUG STATEMENT REMOVE LATER
                #print("Image all zeros state (1): ", (outim_os_or== 0).all())
            #print("Output image (1): ", outim_os_or)
            #print("Output image (1.3): ", outim_os_or.shape)
                #print("\n")"""
            ...
            #TODO: FIX FIX FIX! Please have mercy on us

            for z in range(outim_os_or.shape[3]):
                # Calculate output segmentation mask from class probabilities
                outim[:, :, z] = np.argmax(np.squeeze(outim_os_or[5, :, :, z]), axis=0)

            # TODO: DEBUG STATEMENT REMOVE LATER
            """print("Image all zeros state (2): ", (outim == 0).all())
            print("Output image (2): ", outim)
            print("\n")"""
            ...
            print("Image all zeros state: ", (outimt == 0).all())
